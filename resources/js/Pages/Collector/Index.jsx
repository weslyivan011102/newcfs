...
const Index = () => {
   ...
   const fetchCollectors = async ({ queryKey }) => {
      ...
      const response = await axios.get(`${API_URL}/api/get_collectorx_paginate`, {
   ...
   const [currentPage, setCurrentPage] = useState(collectors?.current_page || 1);

   const { data, isLoading, refetch } = useQuery({
      queryKey: ["collectors", currentPage, searchQuery, sortConfig.column, sortConfig.direction],
   ...
   const TABLE_ROWS =
      data?.data.map((collector) => ({
         id: collector.id,
         collector_name: `${collector.lastname}, ${collector.firstname} ${collector.middlename ?? ""}`,
   ...
   const deleteCollector = async (collectorId) => {
      ...
      try {
         const response = await axios.delete(`/admin/collectors/${collectorId}`);
         ...
         <div className="bg-white overflow-y-auto max-h-[590px]">
            <div className="mt-5 px-4">
               <div className="mb-6 flex justify-between items-center">
                  ...
                  <div>
                     <Link href="/admin/collectors/create">
                        ...
                        TABLE_ROWS.map(
                           ({
                              ...
                              <tr key={id} className="hover:bg-blue-gray-50">
                                 ...
                                 <td className="border border-blue-gray-100 px-4">
                                    <Typography variant="small" className="font-normal text-gray-800">
                                       {format(new Date(birthdate), "MM/dd/yyyy")}
                                    </Typography>
                                 </td>
                                 <td className="border border-blue-gray-100 px-4">
                                    <Typography variant="small" className="font-normal text-gray-800">
                                       ...
                                       <Menu>
                                          ...
                                          <MenuList>
                                             <Link
                                                ...
                                                href={route("collectors.edit", { id })}
                                             >
                                                <MenuItem>Edit</MenuItem>
                                             </Link>
                                             <MenuItem>
                                                <span onClick={() => deleteCollector(id)}>Delete</span>
                                             </MenuItem>
                                          </MenuList>
                                       </Menu>
                                    </div>
